  ...
  function Modal({ title, children, onClose }) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-transparent backdrop-blur-sm">
        <div className="bg-white rounded-xl shadow-xl w-11/12 md:w-4/5 lg:w-3/4 xl:w-2/3 p-6 max-h-[90vh] overflow-y-auto border border-gray-200 relative">
  ...
  const [paymentConferenceFilter, setPaymentConferenceFilter] = useState("All");
  const [paymentFeeTypeFilter, setPaymentFeeTypeFilter] = useState("All");
  const [filteredPayments, setFilteredPayments] = useState([]);
  const uniqueConferences = React.useMemo(() => [...new Set(payments.map(p => p.conferenceName))], [payments]);
  const uniqueFeeTypes = React.useMemo(() => [...new Set(payments.map(p => p.feeType))], [payments]);
  const [paymentSortOrder, setPaymentSortOrder] = useState("Newest"); // "Newest" hoặc "Oldest"
  ...
  const paymentModalPageSize = 5;
    ...
    try {
      const data = await PayService.getPaymentsByUser();
  setPayments(data || []);
  filterPayments(data || []); // chỉ truyền data, logic filter dùng state hiện tại

      setPaymentModalOpen(true);
    } catch (error) {
      console.error("❌ Lỗi khi lấy payment history:", error);
      setPayments([]);
      setFilteredPayments([]);
      setPaymentModalOpen(true);
    } finally {
    ...
    const filterPayments = (paymentsData, conference, feeType, sortOrder) => {
  let filtered = [...paymentsData];

  if (conference && conference !== "All") {
    filtered = filtered.filter(p => p.conferenceName === conference);
  }

  if (feeType && feeType !== "All") {
    filtered = filtered.filter(p => p.feeType === feeType);
  }

  filtered.sort((a, b) => {
    const dateA = dayjs(a.paidAt);
    const dateB = dayjs(b.paidAt);
  ...
  setPaymentModalPage(1);
    ...
    return (
      ...
      className="mt-4 px-6 py-2 rounded-lg bg-gradient-to-r from-green-500 to-teal-600 hover:from-green-600 hover:to-teal-700 text-white font-semibold shadow-md transition duration-300 ease-in-out flex items-center gap-2"
    >
      <FaClipboardCheck /> View Payment History
    </button>
            </div>
  ...
  {paymentModalOpen && (
    <Modal title="💳 Payment History" onClose={() => setPaymentModalOpen(false)}>
      {loadingPayments ? (
  ...
  {/* Conference Filter */}
  <Select
    value={paymentConferenceFilter}
    onChange={(val) => {
      setPaymentConferenceFilter(val);
      filterPayments(payments, val, paymentFeeTypeFilter, paymentSortOrder);
    }}
  ...
  {/* Fee Type Filter */}
  <Select
    value={paymentFeeTypeFilter}
    onChange={(val) => {
      setPaymentFeeTypeFilter(val);
      filterPayments(payments, paymentConferenceFilter, val, paymentSortOrder);
    }}
    className="w-full sm:w-1/3"
    dropdownMatchSelectWidth={false}
  >
    <Option value="All">All Fee Types</Option>
    {uniqueFeeTypes.map(f => <Option key={f} value={f}>{f}</Option>)}
  </Select>
  ...
  <Select
    ...
    onChange={(val) => {
      setPaymentSortOrder(val);
      filterPayments(payments, paymentConferenceFilter, paymentFeeTypeFilter, val);
    }}
    className="w-full sm:w-1/3"
    dropdownMatchSelectWidth={false}
  >
    ...
    <Option value="Oldest">Oldest</Option>
          ...
          <table className="w-full border border-gray-200 rounded mb-4 text-sm md:text-base">
            ...
            <tbody>
              {filteredPayments
                .slice(
                  (paymentModalPage - 1) * paymentModalPageSize,
                  paymentModalPage * paymentModalPageSize
                )
                .map((p) => (
                  <tr key={p.payId} className="border-t border-gray-200 hover:bg-gray-50">
                    <td className="px-2 py-1">{p.conferenceName}</td>
                    <td className="px-2 py-1">{p.paperTitle || '-'}</td>
                    <td className="px-2 py-1">{p.amount} {p.currency}</td>
                    <td className="px-2 py-1">{p.feeType || '-'}</td>
                    <td className="px-2 py-1">{p.mode || '-'}</td>
                    <td className="px-2 py-1">
                      {p.paidAt ? dayjs(p.paidAt).format('DD/MM/YYYY HH:mm') : '-'}
                    </td>
                  </tr>
                ))}
            </tbody>
          ...
          {/* Pagination modal Payment History */}
          <div className="flex justify-center">
            <Pagination
              current={paymentModalPage}
              pageSize={paymentModalPageSize}
              total={filteredPayments.length}
              onChange={(page) => setPaymentModalPage(page)}
              showSizeChanger={false}
              simple
            />
          </div>
